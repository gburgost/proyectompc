<?php
	...
	$myusuario = mysqli_query($conexion, "SELECT rut_persona, nombre, apellido FROM persona WHERE tipo_persona = 'Visita' AND rut_persona =  '".htmlentities($_POST["rut"])."'");
          ...
          $nmyclave = mysqli_num_rows($myclave);
          	...
          	//Se Valida el estado de la persona y se compara con el rut
          	 if ($nmyclave != 0){
		         ...
		         //Si el estado esta en "abierto" se genera una alerta
		          ...
		          else
		          {
    				...
    				$foto = "";
          	 		...
          	 		mysqli_query($conexion, "INSERT INTO visita(id_visita, rut_persona, nombre_visitado, empresa) VALUES('', '$rut_persona', '$visitado', '$empresa')");
            	...
            	<div class="imguser"><p>'.$tipopersona.'</p></div></div>';
            	echo "<script>setTimeout('document.location.reload()',5000);</script>";
		          }
          }
          else{
          	$estado = "abierto";
          	 mysqli_query($conexion, "INSERT INTO registro_persona (cod_registro, nro_garita, rut_persona, rut_guardia, fecha_entrada, hora_entrada, fecha_salida, hora_salida, estado) VALUES('', '$nro_garita', '$rut_persona', '$rut_guardia', '$fecha', '$hora', '', '', '$estado' )");
            	echo '<div class="alert alert-success" role="alert"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> Registro de entrada exitoso.</div>';
            	echo "<script>setTimeout('document.location.reload()',5000);</script>";
          }
     }else{
